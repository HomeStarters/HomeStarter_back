...
UFR-ASST-040: [자산정보관리] 사용자로서 | 나는, 가구 전체 재무상태를 반영하기 위해 | 본인 자산정보와 가구원들의 자산정보를 모두 조회하고 싶다.
- 시나리오: 자산정보 조회
  로그인한 상태에서 자산정보 관리 화면 또는 대시보드 화면에 접근한 상황에서 | 본인/가구원 자산정보를 확인한다.

  [조회 기능]
  - [ ] 본인과 가구원의 자산정보는 별도 구분하여 표시
  - [ ] 본인과 가구원 모두 카테고리별 목록 표시 (자산/대출/월소득/월지출)
  - [ ] 본인과 가구원 모두 총액 자동 계산 표시
  - [ ] 본인과 가구원 모두 재무 요약 정보 표시 (순자산, 월 가용자금 등)

  [수정 기능]
  - [ ] 본인 자산정보에 대한 수정
  - [ ] 수정버튼 클릭 시 본인 자산정보 입력 화면으로 이동
